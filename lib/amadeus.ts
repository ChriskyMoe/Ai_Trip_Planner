...
const FALLBACK_AIRPORTS = [
  {
    ...
    name: { text: "Suvarnabhumi International Airport" },
  ...
  {
    iataCode: "CDG",
    detailedName: "Charles de Gaulle Airport",
    name: { text: "Charles de Gaulle Airport" },
    address: { cityName: "Paris", countryName: "France" },
  },
  {
    iataCode: "ORY",
    detailedName: "Paris Orly Airport",
    name: { text: "Paris Orly Airport" },
    address: { cityName: "Paris", countryName: "France" },
  },
  {
    iataCode: "JFK",
    detailedName: "John F. Kennedy International Airport",
    name: { text: "John F. Kennedy International Airport" },
    address: { cityName: "New York", countryName: "United States" },
  },
  {
    iataCode: "LGA",
    detailedName: "LaGuardia Airport",
    name: { text: "LaGuardia Airport" },
    address: { cityName: "New York", countryName: "United States" },
  },
  {
    iataCode: "HND",
    detailedName: "Haneda Airport",
    name: { text: "Haneda Airport" },
    address: { cityName: "Tokyo", countryName: "Japan" },
  },
  {
    iataCode: "NRT",
    detailedName: "Narita International Airport",
    name: { text: "Narita International Airport" },
    address: { cityName: "Tokyo", countryName: "Japan" },
  },
  {
    iataCode: "LHR",
    detailedName: "London Heathrow Airport",
    name: { text: "London Heathrow Airport" },
    address: { cityName: "London", countryName: "United Kingdom" },
  },
  {
    iataCode: "LGW",
    detailedName: "London Gatwick Airport",
    name: { text: "London Gatwick Airport" },
    address: { cityName: "London", countryName: "United Kingdom" },
  },
];

function filterFallbackAirports(query: string) {
  const q = query.trim().toLowerCase();
  if (!q) return [];
  return FALLBACK_AIRPORTS.filter((airport) => {
    const codeMatch = airport.iataCode.toLowerCase().includes(q);
    const nameMatch =
      airport.detailedName.toLowerCase().includes(q) ||
      (typeof airport.name === "string"
        ? airport.name.toLowerCase().includes(q)
        : airport.name?.text?.toLowerCase().includes(q));
    const cityMatch = airport.address?.cityName?.toLowerCase().includes(q);
    const countryMatch = airport.address?.countryName
      ?.toLowerCase()
      .includes(q);
...
async function getAccessToken(): Promise<string> {
  ...
  if (!AMADEUS_API_KEY || !AMADEUS_API_SECRET) {
    throw new Error(
      "AMADEUS_API_KEY and AMADEUS_API_SECRET must be configured"
    );
  }
