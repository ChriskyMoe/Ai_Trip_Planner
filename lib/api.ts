const LITEAPI_KEY = process.env.LITE_API_KEY;
const API_BASE = "https://api.liteapi.travel/v3.0";
const BOOK_BASE = "https://book.liteapi.travel/v3.0";

if (!LITEAPI_KEY) {
  throw new Error("LITE_API_KEY environment variable is not set");
}
...
export async function bookHotel(params: {
  ...
  console.log(
    "Calling LiteAPI book endpoint with:",
  ...
  if (!response.ok) {
    ...
    const errorMessage =
      error.error?.message || error.message || "Failed to book";
